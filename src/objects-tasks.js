...
function removeProperties(obj, keys) {
  ...
  Object.keys(res).forEach((key) => (keys.includes(key) ? delete res[key] : 0));
...
function makeWord(lettersObject) {
  const resLength = Object.values(lettersObject).flat().length;
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i] > price) {
      if (queue[i] - price > ticketOffice) {
        return false;
      }
    }
    ticketOffice += queue[i];
  }
  return true;
}
